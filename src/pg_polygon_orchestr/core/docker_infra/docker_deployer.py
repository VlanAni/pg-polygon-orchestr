...
class DockerDeployer(Deployer):
    ...
    def __init__(self, id: uuid.UUID | None = None) -> None:
        ...
        self.__docker_session: DockerClientSession = DockerClientSession()
    ...
    def put_node_config(self, name: str, config: NodeConfig) -> Node:
        ...
        if self.__docker_nodes.put_object_in_registry(entity=d_node):
            return d_node
        else:
            return typing.cast(
                DockerNode,
    ...
    def put_network_config(self, name: str, config: NetConfig) -> Network:
        ...
        if self.__docker_networks.put_object_in_registry(entity=d_net):
            return d_net
        else:
            return typing.cast(
                DockerNetwork,
    ...
    def __freeze_nodes_before_snapshot(self, stop_timeout: int) -> list[DockerNode]:
        nodes_registry_size = len(self.__docker_nodes.get_uuid_map())
        stopped_nodes: list[DockerNode] = list()

        with pool.ThreadPoolExecutor(max_workers=nodes_registry_size) as executor:
            future_to_nodes: dict[pool.Future[None], DockerNode] = {}
    ...
    def __restart_stopped_nodes_after_snapshot(
        self, stopped_nodes: list[DockerNode]
    ) -> None:
